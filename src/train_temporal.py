...
from FusionNet_temporal_transformer import TemporalTransformerScorer
# =========================
# IO Utils
# =========================
...
class LessonDataset(Dataset):
    def __init__(self, label_dir, visual_dir, audio_dir,CONTEXT_RADIUS=8):
        self.samples = []
        self.CONTEXT_RADIUS = CONTEXT_RADIUS              # 前后各 8 个窗口
        self.SEQ_LEN = 2 * CONTEXT_RADIUS + 1  # 总长度 17
        ...
        for label_file in label_files:
            ...
            for idx in range(num_windows):
                ...
                if np.isnan(seq_features).any():
                    continue
                self.samples.append({
                    "features": torch.tensor(seq_features, dtype=torch.float32),  # [T, C]
            ...
            print(f"  提取 {video_count} 个中心窗口时序样本，每个样本长度={self.SEQ_LEN}")
...
def train_model():
    ...
    print("📊 特征与标签的相关系数:")
    all_features = torch.stack([s["features"] for s in dataset.samples])  # [N, T, C]
    all_labels = torch.tensor([s["label"].item() for s in dataset.samples])  # [N]

    center_idx = all_features.shape[1] // 2
    center_features = all_features[:, center_idx, :]  # [N, C]

    print("📊 中心窗口特征与标签的相关系数:")
    for i in range(center_features.shape[1]):
        x = center_features[:, i].numpy()
        y = all_labels.numpy()
    ...
    model = TemporalTransformerScorer(
        input_dim=input_dim,
        d_model=64,
        nhead=4,
        num_layers=2,           # ← 通过此参数便捷修改层数
        dim_feedforward=128,
        dropout=0.1,
        use_pos_enc=True
    ).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        for features, labels in train_loader:
            ...
            outputs = model(features)
             ...
             # 根据模型类型选择输出
            if isinstance(model, TemporalTransformerScorer):
                # 输出已经是 [B, 1]
                center_outputs = outputs
            else:
                # 其他模型输出 [B, T, 1]，取中心
                center_idx = outputs.shape[1] // 2
                center_outputs = outputs[:, center_idx, :]

            loss = criterion(center_outputs, labels)
            loss.backward()
            optimizer.step()
